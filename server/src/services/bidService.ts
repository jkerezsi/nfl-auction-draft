...
export function submitBid(
  ...
  if (!game || game.status !== "AUCTION") {
    throw new Error("There is no active auction");
  }
...
export function resolveAuction(
  ...
  const resolveTransaction = db.transaction(() => {
    ...
    if (!game || game.status !== "AUCTION") {
      throw new Error("There is no active auction to resolve");
    }


    if (game.currentPlayerId !== playerId) {
      throw new Error("This is not the current auction player");
    }
    ...
    const team = db
      ...
      .get(winner.team_id) as TeamRow | undefined;
    ...
    if (winner.amount > team.budget) {
      throw new Error("Winning bid exceeds the team's current budget");
    }


    const budgetUpdate = db
      .prepare(
        `
        UPDATE teams
        SET budget = budget - ?
    ...
    if (budgetUpdate.changes !== 1) {
      throw new Error("Could not deduct the winning bid");
    }


    const draftedUpdate = db
      .prepare(
        `
